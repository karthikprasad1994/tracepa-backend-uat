...
namespace TracePca.Service.Audit
{
    public class EngagementPlanService : EngagementPlanInterface
    {
        ...
        public async Task<(byte[] fileBytes, string contentType, string fileName)> GenerateDocumentAsync(EngagementPlanDetailsDTO data, string fileType)
        {
            return fileType.ToLower() switch
            {
                "pdf" => (await GeneratePdfAsync(data), "application/pdf", "EngagementLetter.pdf"),
                "word" => (await GenerateWordAsync(data),
                           "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                           "EngagementLetter.docx"),
                _ => throw new ArgumentException("Invalid file type. Use 'pdf' or 'word'.")
            };
        }

        private async Task<byte[]> GeneratePdfAsync(EngagementPlanDetailsDTO data)
        {
            return await Task.Run(() =>
            {
                ...
                document.Add(new Paragraph("ENGAGEMENT LETTER", headerFont) { Alignment = Element.ALIGN_CENTER });
        ...
        private async Task<byte[]> GenerateWordAsync(EngagementPlanDetailsDTO data)
        {
            return await Task.Run(() =>
            {
                using var ms = new MemoryStream();
